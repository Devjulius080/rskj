...
public class JsonRpcCustomServer extends JsonRpcBasicServer {
    ...
    private int getTimeoutByModule(String method) {
        if (method.isEmpty()) {
        ...
        return optModule.map(ModuleDescription::getTimeout).orElseGet(this.rskSystemProperties::getRpcTimeout);
    }
    ...
    protected JsonResponse handleJsonNodeRequest(final JsonNode node) throws JsonParseException, JsonMappingException {
        int timeout = this.getTimeoutByModule(
